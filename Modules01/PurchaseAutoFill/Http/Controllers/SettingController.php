...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/PurchaseAutoFill/Resources/lang/kh/lang.php');
    }

    public function showPurchaseAutoFillPermissionForm()
    {
        ...
        if (!file_exists($this->langFile)) {
            return view('purchaseautofill::PurchaseAutoFill.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('purchaseautofill::PurchaseAutoFill.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $PurchaseAutoFill = PurchaseAutoFillSocial::where('business_id', $business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('purchaseautofill::PurchaseAutoFill.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        if ($request['social_status'] == 1) {
            ...
            // $baseUrl = "https://27052e3b313b.ngrok-free.app";
            $webhookUrl = "{$baseUrl}/telegram/{$business_id}/webhook";
        ...
        $module = PurchaseAutoFillSocial::firstOrNew([
