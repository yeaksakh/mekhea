            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#permission_settings" data-toggle="tab" aria-expanded="true">
                            ...
                            @lang('purchaseautofill::lang.permission_settings')
                        </a>
                    </li>
                    ...
                    <li>
                        <a href="#language_keys" data-toggle="tab" aria-expanded="false">
                            ...
                            @lang('purchaseautofill::lang.update_language')
                        </a>
                    </li>
                    <li>
                        <a href="#telegram_setting" data-toggle="tab" aria-expanded="false">
                            <i class="fab fa-telegram"></i>
                            @lang('purchaseautofill::lang.telegram')
                        </a>
                    </li>
                </ul>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="permission_settings">
                        {!! Form::open(['route' => 'PurchaseAutoFill.permission', 'method' => 'post']) !!}
                            <div class="row">
                                <div class="col-md-12">
                                    <h3> @lang('purchaseautofill::lang.permission_settings')</h3>
                    ...
                    <div class="tab-pane" id="telegram_setting">
                        ...
                        @else
                        <form method="POST" action="{{ route('PurchaseAutoFill.update-social') }}">
                            @csrf
                            <div class="form-group row align-items-center mb-3">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" class="form-control" id="social_token"
                                        placeholder="7978956020:AAG4JAzdlE1MxnuMGhBtHuT8jLZ3a38UEQY" value="{{ $PurchaseAutoFill->social_token ?? '' }}"
                                        name="social_token">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" class="form-control" id="social_id"
                                        placeholder="-1002332935570" value="{{ $PurchaseAutoFill->social_id ?? '' }}"
                                        name="social_id">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="social_status" name="social_status"
                                            value="1" {{ optional($PurchaseAutoFill)->social_status ? 'checked' : '' }}>
                                        <label class="form-check-label" for="social_status">
                                            Enable Webhook
                                        </label>
                                        <small class="form-text text-muted">Check to enable Telegram Webhook so when new image are sent to the group it will auto matical send to the server and store the imag for ocr</small>
                                    </div>
                                </div>
                                <div class="form-group row align-items-center">
                                    <div class="col-sm-6">
                    ...
                    <div class="tab-pane" id="language_keys">
                        ...
                        <h1>@lang('purchaseautofill::lang.language_settings')</h1>
                        @if (session('success'))
                            <p class="text-success">{{ session('success') }}</p>
                        @endif
                        @if (isset($error))
                            <p class="text-danger">{{ $error }}</p>
                        @else
                            <form method="POST" action="{{ route('PurchaseAutoFill.update-language') }}">
                                @csrf
                                <div class="form-group">
                                    <label for="your_language">@lang('purchaseautofill::lang.select_language'):</label>
                                ...
                                <div class="form-group mt-3 text-center">
                                    <button type="submit" class="btn btn-primary">@lang('purchaseautofill::lang.update_language')</button>
                                </div>
                            </form>
                        @endif

                        <h1>@lang('purchaseautofill::lang.language_keys')</h1>
                        @if (session('success'))
                            <p class="text-success">{{ session('success') }}</p>
                        @endif
                        @if (isset($error))
                            <p class="text-danger">{{ $error }}</p>
                        @else
                            <form method="POST" action="{{ route('PurchaseAutoFill.lang') }}">
                                @csrf
                                <table class="table border-0">
                                    <thead>
                                        <tr class="border-0">
                                            <th class="border-0">@lang('purchaseautofill::lang.key')</th>
                                            <th class="border-0">@lang('purchaseautofill::lang.translation')</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                ...
                                <div class="form-group mt-3 text-center">
                                    <button type="submit" the class="btn btn-primary">@lang('purchaseautofill::lang.save_translations')</button>
                                </div>
                            </form>
                        @endif
                    </div>
