...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/AuditExpense/Resources/lang/kh/lang.php');
    }

    public function showAuditExpensePermissionForm()
    {
        ...
        $permissions = Permission::where('id', 678)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('auditexpense::AuditExpense.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('auditexpense::AuditExpense.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $AuditExpense = AuditExpenseSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('auditexpense::AuditExpense.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = AuditExpenseSocial::firstOrNew([
