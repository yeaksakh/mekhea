...
namespace Modules\AuditExpense\Utils;
...
use Modules\AuditExpense\Utils\Util;
...
class TransactionUtil extends Util
{
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        // Check if $transaction_date is already a Carbon object, if not, create one.
        if (!($transaction_date instanceof \Carbon\Carbon)) {
            $transaction_date = \Carbon\Carbon::parse($transaction_date);
        }

        $output['exchange_rate'] = ExchangeRate::where('business_id', $business_id)
            ->where('date_1', $transaction_date->format('Y-m-d')) // Match date_1 to the transaction date
            ->first()
                ?->KHR_3
            ?? 0;
        $output['pay_term_number'] = $transaction->pay_term_number;
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($zatca_qr) {
                ...
                if (in_array('sale_id', $qr_code_fields)) {
                    $qr_code_details[] = $is_label_enabled ? $il->id . ': ' . $transaction->id : $transaction->id;
                }
                if (in_array('sale_type', $qr_code_fields)) {
                    $qr_code_details[] = $is_label_enabled ? $il->type . ': ' . $transaction->type : $transaction->type;
                }
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        $is_warranty_enabled = !empty($business_details->common_settings['enable_product_warranty']);
        ...
        foreach ($lines as $line) {
            ...
            $product_variation = $variation->product_variation;
            $unit = $product->unit;
            $brand = $product->brand;
            ...
            $base_unit_multiplier = $line->multiplier ?? 1;
            ...
            $line_array = [
                ...
                'product_variation' => ($product_variation->name == 'DUMMY') ? '' : $product_variation->name,
                'quantity' => $this->num_f($line->quantity, false, $business_details, true),
                'quantity_uf' => $line->quantity,
                'units' => $unit_name,
                'base_unit_name' => $base_unit_name,
                'base_unit_multiplier' => $this->num_f($base_unit_multiplier, false, $business_details),
                ...
                'tax_percent' => $tax_details->amount ?? null,
                ...
                'brand' => $brand->name ?? '',
                'cat_code' => $cat->short_code ?? '',
                'sell_line_note' => nl2br($line->sell_line_note ?? ''),
                'lot_number' => $line->lot_details->lot_number ?? null,
                'product_expiry' => $line->lot_details->exp_date ?? null,
            ];
    
            // Handle combo products
            if ($product->type == 'combo') {
            	$productUtil = new \App\Utils\ProductUtil();
                $combo_components = $productUtil->__getComboProductDetails(
                    $variation->combo_variations,
                    $business_details->id
                );
    
                $line_components = [];
                foreach ($combo_components as $component) {
                    $component_product = $component['variation']['product'];
                    $component_variation = $component['variation'];
                    
                    $line_components[] = [
                        'name' => $component_product->name,
                        'variation' => $component_variation->name,
                        'sub_sku' => $component_variation->sub_sku,
                        'quantity' => $this->num_f($component['quantity'] * $line->quantity, false, $business_details, true),
                        'unit' => $component['unit_name'],
                        'brand' => $component_product->brand->name ?? '',
                        'custom_fields' => $this->getComponentCustomFields($component_product, $product_custom_fields_settings)
                    ];
                }
                $line_array['combo_components'] = $line_components;
            }
    
            // Handle product custom fields
            $custom_fields = [];
            foreach (['product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4'] as $field) {
                if (!empty($product->$field) && in_array($field, $product_custom_fields_settings)) {
                    $custom_fields[] = $product->$field;
                }
            }
            $line_array['product_custom_fields'] = implode(', ', $custom_fields);
    
            // Handle modifiers
            if (!empty($line->modifiers)) {
                $modifiers = [];
                foreach ($line->modifiers as $modifier) {
                    $modifier_variation = $modifier->variations;
                    $modifiers[] = [
                        'name' => $modifier->product->name,
                        'variation' => $modifier_variation->name,
                        'sub_sku' => $modifier_variation->sub_sku,
                        'quantity' => $this->num_f($modifier->quantity, false, $business_details),
                        'unit' => $modifier->product->unit->short_name,
                    ];
                }
                $line_array['modifiers'] = $modifiers;
            }
    
            // Handle warranties
            if ($is_warranty_enabled && $line->warranties->isNotEmpty()) {
                $warranty = $line->warranties->first();
                $line_array['warranty'] = [
                    'name' => $warranty->display_name,
                    'description' => $warranty->description,
                    'exp_date' => $this->format_date($warranty->getEndDate($line->transaction_date))
                ];
            }
    ...
    protected function getComponentCustomFields($product, $settings)
    {
        $fields = [];
        foreach (['product_custom_field1', 'product_custom_field2', 'product_custom_field3', 'product_custom_field4'] as $field) {
            if (!empty($product->$field) && in_array($field, $settings)) {
    ...
    public function mapPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        ...
        if (!empty($business['pos_settings']['allow_overselling']) || $mapping_type == 'production_purchase') {
        ...
        foreach ($transaction_lines as $line) {
            ...
            $qty_selling = (float) $line->quantity;
            ...
            foreach ($rows as $k => $row) {
                ...
                if ($qty_selling <= (float) $row->quantity_available) {
    ...
    public function createExpense($request, $business_id, $user_id, $status, $format_data = true)
    {
        ...
        $transaction_data['type'] = $request->input('expense', 'expense'); // Default to 'expense'

        if ($request->has('expense_request')) {
            $transaction_data['type'] = $request->input('expense_request');
        }

        if ($request->has('is_refund') && $request->input('is_refund') == 1) {
            $transaction_data['type'] = 'expense_refund';
        }
