<div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title">@lang('auditexpense::lang.edit_auditexpense')</h4>
        </div>
        <div class="modal-body">
            <form id="edit_AuditExpense_form" method="POST" action="{{ route('AuditExpense.update', $auditexpense->id) }}">
                @csrf
                @method('PUT')
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select class="form-control" id="auditexpense_category_id" name="auditexpense_category_id" style="width: 100%;">
                                <option value="">@lang('messages.select')</option>
                                @foreach ($auditexpense_categories as $id => $category)
                                    <option value="{{ $id }}" {{ $auditexpense->category_id == $id ? 'selected' : '' }}>
                                        {{ $category }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="ExpenseSource_1" name="ExpenseSource_1" value="{{ $auditexpense->{'ExpenseSource_1'} }}">
        </div>
    </div>
    <div class="col-md-12">
        <div class="form-group">
            <label for="Amount_2">@lang('auditexpense::lang.Amount_2'):</label>
            <input type="number" class="form-control" id="Amount_2" name="Amount_2" value="{{ $auditexpense->{'Amount_2'} }}" step="any">
        </div>
    </div>
