...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/SOP/Resources/lang/kh/lang.php');
    }

    public function showSOPPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 760)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('sop::SOP.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('sop::SOP.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $SOP = SOPSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('sop::SOP.setting', [
            'roles' => $roles,
