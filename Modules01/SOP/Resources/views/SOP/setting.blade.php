            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    <li class="active">
                        <a href="#permission_settings" data-toggle="tab" aria-expanded="true">
                            ...
                            @lang('sop::lang.permission_settings')
                        </a>
                    </li>
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="permission_settings">
                        {!! Form::open(['route' => 'SOP.permission', 'method' => 'post']) !!}
                            <div class="row">
                                <div class="col-md-12">
                                    <h3> @lang('sop::lang.permission_settings')</h3>
                    ...
                    <div class="tab-pane" id="telegram_setting">
                        ...
                        @else
                        <form method="POST" action="{{ route('SOP.update-social') }}">
                            @csrf
                            <div class="form-group row align-items-center mb-3">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" class="form-control" id="social_token"
                                        placeholder="7978956020:AAG4JAzdlE1MxnuMGhBtHuT8jLZ3a38UEQY" value="{{ $SOP->social_token ?? '' }}"
                                        name="social_token">
                                ...
                                <div class="col-sm-6">
                                    ...
                                    <input type="text" class="form-control" id="social_id"
                                        placeholder="-1002332935570" value="{{ $SOP->social_id ?? '' }}"
                                        name="social_id">
                                ...
                                <div class="col-sm-6">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="social_status" name="social_status"
                                            value="1" {{ optional($SOP)->social_status ? 'checked' : '' }}>
                    ...
                    <div class="tab-pane" id="language_keys">
                        ...
                        <h1>@lang('sop::lang.language_settings')</h1>
                        @if (session('success'))
                            <p class="text-success">{{ session('success') }}</p>
                        @endif
                        @if (isset($error))
                            <p class="text-danger">{{ $error }}</p>
                        @else
                            <form method="POST" action="{{ route('SOP.update-language') }}">
                                @csrf
                                <div class="form-group">
                                    <label for="your_language">@lang('sop::lang.select_language'):</label>
                                ...
                                <div class="form-group mt-3 text-center">
                                    <button type="submit" class="btn btn-primary">@lang('sop::lang.update_language')</button>
                                </div>
                            </form>
                        @endif

                        <h1>@lang('sop::lang.language_keys')</h1>
                        @if (session('success'))
                            <p class="text-success">{{ session('success') }}</p>
                        @endif
                        @if (isset($error))
                            <p class="text-danger">{{ $error }}</p>
                        @else
                            <form method="POST" action="{{ route('SOP.lang') }}">
                                @csrf
                                <table class="table border-0">
                                    <thead>
                                        <tr class="border-0">
                                            <th class="border-0">@lang('sop::lang.key')</th>
                                            <th class="border-0">@lang('sop::lang.translation')</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                ...
                                <div class="form-group mt-3 text-center">
                                    <button type="submit" the class="btn btn-primary">@lang('sop::lang.save_translations')</button>
                                </div>
                            </form>
                        @endif
                    </div>
