...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/BusinessPlanCanvasB1/Resources/lang/kh/lang.php');
    }

    public function showBusinessPlanCanvasB1PermissionForm()
    {
        ...
        $permissions = Permission::where('id', 516)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('businessplancanvasb1::BusinessPlanCanvasB1.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('businessplancanvasb1::BusinessPlanCanvasB1.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Your Language']; // Add more languages as needed

        return view('businessplancanvasb1::BusinessPlanCanvasB1.setting', [
            'roles' => $roles,
