...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/ExpenseRequest/Resources/lang/kh/lang.php');
    }

    public function showExpenseRequestPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 704)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('expenserequest::ExpenseRequest.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('expenserequest::ExpenseRequest.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $ExpenseRequest = ExpenseRequestSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('expenserequest::ExpenseRequest.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = ExpenseRequestSocial::firstOrNew([
