    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('expenserequest::lang.details')</h4>
                <div class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full btn-modal" style="cursor: pointer;" onclick="toggleHideShow()">
                    @lang('expenserequest::lang.filter')
                </div>
            ...
            <div class="modal-body" id="print-content">
                ...
                @if($expenserequest->category)
                    @php
                        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
                        $fileExtension = strtolower(pathinfo($expenserequest->category->image, PATHINFO_EXTENSION));
                    @endphp
                    
                    @if(in_array($fileExtension, $imageExtensions))
                        <label class="form-check-label pt-2 mb-0" for="categoryCheck">
                            <img src="{{ asset('uploads/ExpenseRequestCategory/' . basename($expenserequest->category->image)) }}" 
                                alt="Document Image" 
                                style="max-width: 25px; max-height: 25px; vertical-align: middle;">
                            </label>
                    @elseif($fileExtension === 'pdf')
                        <span class="me-2">
                            <a href="{{ asset('uploads/ExpenseRequestCategory/' . basename($expenserequest->category->image)) }}" 
                            target="_blank" 
                    ...
                    <div class="form-group" id="category-detail">
                        ...
                        <p id="categorydetail" class="form-control-static">{!! $expenserequest->category->description !!}</p>
                    </div>
                    @endif
                </label>
                <!-- Modal Content Goes Here -->
                    <div class="form-group" id="amount_1content">
        <label for="amount_1">@lang('expenserequest::lang.amount_1'):</label>
        <p id="amount_1" class="form-control-static">{{ $expenserequest->{'amount_1'} }}</p>
    </div>
    <div class="form-group" id="expense_for_2content">
        <label for="expense_for_2">@lang('expenserequest::lang.expense_for_2'):</label>
        <p id="expense_for_2" class="form-control-static">{{ $expenserequest->{'expense_for_2'} }}</p>
    </div>
    <div class="form-group" id="who_request_expense_3content">
        <label for="who_request_expense_3">@lang('expenserequest::lang.who_request_expense_3'):</label>
        <p id="who_request_expense_3" class="form-control-static">{{ $expenserequest->whorequestexpense3->first_name . ' ' . $expenserequest->whorequestexpense3->last_name }}</p>
    </div>
                ...
                <div class="form-group" id="createdatcontent">
                    ...
                    <p id="createdat" class="form-control-static">{{ $expenserequest->created_at }}</p>
                </div>
                <div class="form-group" id="qrcontent">
                    <label for="qrcode">@lang('expenserequest::lang.qrcode'):</label>
