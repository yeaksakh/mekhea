@extends('layouts.app')
@section('title', __('expense.expenses_request'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1 class="tw-text-xl md:tw-text-3xl tw-font-bold tw-text-black">@lang('expense.expenses_request')</h1>
</section>
...
<section class="content">
    <div class="row">
		...
		<div class="col-md-3">
		    <div class="form-group">
		        ...
		        {!! Form::select('audit_status', $audit_statuses, null, ['class' => 'form-control select2', 'id' => 'audit_status', 'style' => 'width:100%', 'placeholder' => __('lang_v1.all'),]) !!}
		    </div>
		</div>
            @endcomponent
        </div>
    ...
    <div class="d-flex flex-nowrap overflow-auto" style="gap: 60px;">
        ...
        @if (auth()->user()->can('purchase.view') || auth()->user()->can('view_own_purchase'))
            <button onclick="window.location.href='{{ action([\App\Http\Controllers\PurchaseController::class, 'index']) }}'" class="btn btn-primary rounded-2">
        ...
        @if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own'))
            <button onclick="window.location.href='{{ action([\App\Http\Controllers\ContactController::class, 'index'], ['type' => 'supplier']) }}'" class="btn btn-success rounded-2">
                <i class="fas fa-hiking fa-fw"></i> {{ __('report.supplier') }}
            </button>
        @endif
            <!-- Expense Categories -->
        @if (auth()->user()->can('expense.add') || auth()->user()->can('expense.edit'))
            <button onclick="window.location.href='{{ action([\App\Http\Controllers\ExpenseCategoryController::class, 'index']) }}'" class="btn btn-warning rounded-2">
                <span>
...
@endcomponent
    ...
    <div class="row">
        <div class="col-md-12">
            @component('components.widget', ['class' => 'box-primary', 'title' => __('expense.all_expenses_request')])
                @can('expense.add')
                    @slot('tool')
                        <div class="box-tools">
                            ...
                            <a class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right"
                                href="{{action([\App\Http\Controllers\ExpenseController::class, 'create'])}}?request=true">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped" id="expense_request_table">
                        <thead>
                            <tr>
...
<div class="modal fade view_expent_modal" tabindex="-1" role="dialog" 
    aria-labelledby="gridSystemModalLabel">
</div>
@stop
@section('javascript')
