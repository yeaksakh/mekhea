...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/SWOT/Resources/lang/kh/lang.php');
    }

    public function showSWOTPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 772)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('swot::SWOT.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('swot::SWOT.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $SWOT = SWOTSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('swot::SWOT.setting', [
            'roles' => $roles,
