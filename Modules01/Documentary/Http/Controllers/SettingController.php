...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/Documentary/Resources/lang/kh/lang.php');
    }

    public function showDocumentaryPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 902)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('documentary::Documentary.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('documentary::Documentary.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $Documentary = DocumentarySocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('documentary::Documentary.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = DocumentarySocial::firstOrNew([
