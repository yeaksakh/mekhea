...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/CustomerCardB1/Resources/lang/kh/lang.php');
    }

    public function showCustomerCardB1PermissionForm()
    {
        ...
        $permissions = Permission::where('id', 683)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('customercardb1::CustomerCardB1.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('customercardb1::CustomerCardB1.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $CustomerCardB1 = CustomerCardB1Social::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('customercardb1::CustomerCardB1.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = CustomerCardB1Social::firstOrNew([
