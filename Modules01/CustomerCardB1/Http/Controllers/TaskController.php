...
use Modules\CustomerCardB1\Entities\ProjectTask;
use Modules\CustomerCardB1\Utils\ProjectUtil;
...
class TaskController extends Controller
{
            ...
            if (request()->get('task_view') == 'list_view') {
                return Datatables::of($project_task)
                    ->addColumn('action', function ($row) use ($can_crud) {
                        $html = '<div class="btn-group">
                                    <button class="tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-info tw-w-max dropdown-toggle " type="button"  data-toggle="dropdown" aria-expanded="false">
                                      ...
                                      <ul class="dropdown-menu dropdown-menu-left" role="menu">
                                       <li>
                                            <a data-href="'.action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'show'], [$row->id, 'project_id' => $row->project_id]).'" class="cursor-pointer view_a_project_task">
                                                <i class="fa fa-eye"></i>
                                                '.__('messages.view').'
                                            </a>
                                        </li>
                                        <li>
                                            <a data-href="'.action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'getTaskStatus'], ['id' => $row->id, 'project_id' => $row->project_id]).'"class="cursor-pointer change_status_of_project_task">
                        ...
                        if ($can_crud) {
                            $html .= '<li>
                                    <a data-href="'.action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'edit'], [$row->id, 'project_id' => $row->project_id]).'" class="cursor-pointer edit_a_project_task">
                                        <i class="fa fa-edit"></i>
                                        '.__('messages.edit').'
                                    </a>
                                </li>
                                <li>
                                    <a data-href="'.action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'destroy'], [$row->id, 'project_id' => $row->project_id]).'" class="cursor-pointer delete_a_project_task">
                    ...
                    ->editColumn('start_date', function ($row) {
                        ...
                        return \Carbon\Carbon::parse($row->start_date)->format('d-m-Y H:i'); // Shows "08-06-2024 03:07"
                    })
                    ->editColumn('due_date', function ($row) {
                        if (empty($row->due_date)) {
                            return '-';
                        }
                        return \Carbon\Carbon::parse($row->due_date)->format('d-m-Y H:i'); // Shows "08-06-2024 03:07"
                    })
                    ...
                    ->editColumn('members', function ($row) {
                        ...
                        foreach ($row->members as $member) {
                            $name = e($member->user_full_name);
                            $url = action([\App\Http\Controllers\ContactController::class, 'show'], [$member->id]);
                    ...
                    ->editColumn('status', function ($row) {
                        ...
                        $href = action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'getTaskStatus'], ['id' => $row->id, 'project_id' => $row->project_id]);
                    ...
                    ->editColumn('subject', '
                            <a data-href="{{action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, \'show\'], [$id, "project_id" => $project_id])}}" class="cursor-pointer view_a_project_task text-black">
                                {{$subject}} <code>{{$task_id}}</code>
                            </a>
                        ')
                    
                    ->rawColumns(['action', 'project', 'subject', 'members', 'priority', 'start_date', 'due_date', 'status', 'createdBy', 'description'])
                    ->make(true);
                ...
                foreach ($project_tasks as $key => $tasks) {
                    ...
                    foreach ($tasks as $task) {
                        ...
                        $delete = '';
                        if ($can_crud) {
                            $edit = action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'edit'], [$task->id, 'project_id' => $task->project_id]);

                            $delete = action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'destroy'], [$task->id, 'project_id' => $task->project_id]);
                        }

                        $view = action([\Modules\CustomerCardB1\Http\Controllers\TaskController::class, 'show'], [$task->id, 'project_id' => $task->project_id]);
    ...
    public function store(Request $request)
    {
        try {
            ...
            // $input['start_date'] = ! empty($request->input('start_date')) ? $this->commonUtil->uf_date($request->input('start_date')) : true;
            // $input['due_date'] = ! empty($request->input('due_date')) ? $this->commonUtil->uf_date($request->input('due_date')) : true;

            $input['start_date'] = $this->commonUtil->uf_date($request->input('start_date'), true);
            $input['due_date'] = $this->commonUtil->uf_date($request->input('due_date'), true);
