...
namespace Modules\CustomerCardB1\Utils;
...
class ContactUtil extends Util
{
    ...
    public function createNewContact($input, $request = null)
    {
        //Check Contact id
        $count = 0;
    ...
    public function updateContact($input, $id, $business_id, $request = null)
    {
        try {
            $count = 0;
            ...
            if ($count > 0) {
                return [
                    'success' => false,
                    'msg' => __('contact.duplicate_contact_id'),
                ];
            }
            ...
            if ($request) {
                // Check if an old photo exists (though this is a new contact, keeping for consistency)
                if ($contact->media()->where('model_media_type', 'profile_photo')->exists()) {
                    $oldMedia = $contact->media()->where('model_media_type', 'profile_photo')->first();
                    if ($oldMedia) {
            ...
            return [
                'success' => false,
                'msg' => __('contact.update_failed') . ': ' . $e->getMessage(),
            ];
        }
    }
    ...
    public function assignTo($contact)
{
    ...
    // Format the names and include designation and department
    $formattedData = $contacts->map(function ($user) {
        $surname = trim($user->surname ?? '');
        $firstName = trim($user->first_name ?? '');
        $lastName = trim($user->last_name ?? '');

        // Fetch designation and department from Category model
        $user_designation = Category::find($user->essentials_designation_id);
        $user_department = Category::find($user->essentials_department_id);

        return [
            'id' => $user->id,
            'name' => implode(' ', array_filter([$surname, $firstName, $lastName])),
            'designation' => $user_designation ? $user_designation->name : '-', // Fallback if not found
            'department' => $user_department ? $user_department->name : '-'   // Fallback if not found
        ];
    })->all();
