    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('customercardb1::lang.details')</h4>
                <div class="tw-dw-btn tw-bg-gradient-to-r tw-from-indigo-600 tw-to-blue-500 tw-font-bold tw-text-white tw-border-none tw-rounded-full btn-modal" style="cursor: pointer;" onclick="toggleHideShow()">
                    @lang('customercardb1::lang.filter')
                </div>
            ...
            <div class="modal-body" id="print-content">
                ...
                @if($customercardb1->category)
                    @php
                        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
                        $fileExtension = strtolower(pathinfo($customercardb1->category->image, PATHINFO_EXTENSION));
                    @endphp
                    
                    @if(in_array($fileExtension, $imageExtensions))
                        <label class="form-check-label pt-2 mb-0" for="categoryCheck">
                            <img src="{{ asset('uploads/CustomerCardB1Category/' . basename($customercardb1->category->image)) }}" 
                                alt="Document Image" 
                                style="max-width: 25px; max-height: 25px; vertical-align: middle;">
                            </label>
                    @elseif($fileExtension === 'pdf')
                        <span class="me-2">
                            <a href="{{ asset('uploads/CustomerCardB1Category/' . basename($customercardb1->category->image)) }}" 
                            target="_blank" 
                    ...
                    @endif
                    <span>{{ $customercardb1->category->name }}</span>
                    <div class="form-group" id="category-detail">
                        <label for="categorydetail">@lang('employeecontractb1::lang.description'):</label>
                        <p id="categorydetail" class="form-control-static">{!! $customercardb1->category->description !!}</p>
                    </div>
                    @endif
                </label>
                <!-- Modal Content Goes Here -->
                    <div class="form-group" id="customer_1content">
        <label for="customer_1">@lang('customercardb1::lang.customer_1'):</label>
        <p id="customer_1" class="form-control-static">{{ $customercardb1->customer1->name }}</p>
    </div>
                ...
                <div class="form-group" id="createdatcontent">
                    ...
                    <p id="createdat" class="form-control-static">{{ $customercardb1->created_at }}</p>
                </div>
                <div class="form-group" id="qrcontent">
                    <label for="qrcode">@lang('customercardb1::lang.qrcode'):</label>
