 ...
 public function handleWebhook(Request $request, $businessId)
{
    try {
        ...
        $botToken  = ExpenseAutoFillSocial::where('business_id', $businessId)
            ->value('social_token');
        ...
        $isMediaGroup = isset($update['message']['media_group_id']);
        $mediaGroupId = $isMediaGroup ? $update['message']['media_group_id'] : null;
        $messageId = $update['message']['message_id'];
        $chatId = $update['message']['chat']['id'];
        
        // Check if this specific message has already been processed
        $existingRecord = TelegramExpenseImageData::where('telegram_message_id', $messageId)
        ...
        // For media groups, we only want to send the "processing" message once
        $sendMessage = true;
        if ($isMediaGroup) {
            $cacheKey = "telegram_media_group_msg_{$businessId}_{$mediaGroupId}";
            if (Cache::has($cacheKey)) {
                $sendMessage = false;
            } else {
                // Mark this media group as having received the initial message for 5 minutes
        ...
        // Handle PHOTOS (including media groups)
    ...
    private function sendMessage($chatId, $message, $botToken)
    {
        ...
        if (!$response->successful()) {
            \Log::error('Failed to send Telegram message: ' . $botToken . $response->body());
        }
    ...
    protected function processImage($message, $businessId, $botToken)
    {
        try {
            ...
            $existingFile = TelegramExpenseImageData::where('telegram_file_unique_id', $fileUniqueId)
                ->where('business_id', $businessId)
                ->first();
            ...
            // Create record in TelegramExpenseImageData table
            $imageData = TelegramExpenseImageData::create([
                ...
                'telegram_user_last_name' => $message['from']['last_name'] ?? null,
                'telegram_user_username' => $message['from']['username'] ?? null,
                'telegram_user_photo_url' => null, // Would need additional API call to get this
            ...
            \Log::info('Database record created in TelegramExpenseImageData', [
                'id' => $imageData->id,
                'messageId' => $messageId
            ]);

            // Now process with OCR
            $this->processWithOcr($imageData, $chatId, $botToken);
        } catch (Exception $e) {
     ...
     */
    protected function processWithOcr($imageData, $chatId, $botToken)
    {
        try {
            $imagePath = public_path($imageData->file_path);
            ...
            // Prepare telegram data for OCR service
            $telegramData = [
                'file_id' => $imageData->telegram_file_id,
                'file_unique_id' => $imageData->telegram_file_unique_id,
                'file_size' => $imageData->telegram_file_size,
                'file_name' => $imageData->telegram_file_name,
                'width' => $imageData->telegram_width,
                'height' => $imageData->telegram_height,
                'user_id' => $imageData->telegram_user_id,
                'first_name' => $imageData->telegram_user_first_name,
                'last_name' => $imageData->telegram_user_last_name,
                'username' => $imageData->telegram_user_username,
                'photo_url' => $imageData->telegram_user_photo_url,
                'date' => $imageData->telegram_date->format('Y-m-d H:i:s'),
                'message_id' => $imageData->telegram_message_id,
                'chat_id' => $imageData->telegram_chat_id,
                'file_path' => $imageData->file_path
            ];

            // No need to pass business_id anymore, the OCR service will get it from bot token
            $ocrResult = $this->ocrService->extractInvoiceData($base64Image, $telegramData, $botToken);
            ...
            if ($ocrResult['success']) {
                ...
                if (isset($ocrResult['data']['supplier'])) {
                    $message .= "Supplier: " . $ocrResult['data']['supplier'] . "\n";
                }
                if (isset($ocrResult['data']['ref_no'])) {
                    $message .= "Reference: " . $ocrResult['data']['ref_no'] . "\n";
                }
                if (isset($ocrResult['data']['total_amount'])) {
                    $message .= "Total: " . $ocrResult['data']['total_amount'] . "\n";
                }
