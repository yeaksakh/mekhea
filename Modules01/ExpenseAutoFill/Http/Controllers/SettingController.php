...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/ExpenseAutoFill/Resources/lang/kh/lang.php');
    }

    public function showExpenseAutoFillPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 903)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('expenseautofill::ExpenseAutoFill.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('expenseautofill::ExpenseAutoFill.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $ExpenseAutoFill = ExpenseAutoFillSocial::where('business_id', $business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('expenseautofill::ExpenseAutoFill.setting', [
            'roles' => $roles,
    ...
    public function updateSocial(Request $request)
    {
        ...
        if ($request['social_status'] == 1) {
            ...
            // $baseUrl = "https://7c10f58ac9dc.ngrok-free.app";
            $webhookUrl = "{$baseUrl}/telegram/{$business_id}/webhook-expense";
        ...
        $module = ExpenseAutoFillSocial::firstOrNew([
