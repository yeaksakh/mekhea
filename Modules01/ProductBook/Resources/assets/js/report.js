$(document).ready(function() {
    ...
    if ($('table#product_sell_report_table').length == 1) {
        ...
        $('#psr_filter_location_id, #only_profit',
        ).change(function() {
            product_sell_report.ajax.reload();
            product_sell_grouped_report.ajax.reload();
            product_sell_report_with_purchase_table.ajax.reload();
        ...
        $(document).on('ifChanged', '#only_profit', function() {
            product_sell_report.ajax.reload();
            product_sell_grouped_report.ajax.reload();
            product_sell_report_with_purchase_table.ajax.reload();
            $('.nav-tabs li.active').find('a[data-toggle="tab"]').trigger('shown.bs.tab');
        ...
        product_sell_report = $('table#product_sell_report_table').DataTable({
            ...
            aaSorting: [[6, 'desc']],
            ajax: {
                url: '/minireportb1/standardreport/product-sell-report',
                data: function(d) {
                    var start = '';
                    var end = '';
                    ...
                    if ($('#product_sr_date_filter').val()) {
                        var daterangepicker = $('input#product_sr_date_filter').data('daterangepicker');
                    ...
                    d.location = $('select#psr_filter_location_id').val();
                    if ($('#only_profit').length == 1 && $('#only_profit').is(
                        ':checked')) {
                        d.only_profit = 1;
                    }
                },
            ...
            columns: [
                ...
                { data: 'subtotal', name: 'subtotal', searchable: false },
                { data: 'total_depot_price', name: 'total_depot_price', searchable: false },
            ...
            fnDrawCallback: function(oSettings) {
                ...
                $('#footer_total_depot_price').html(
                    __currency_trans_from_en(sum_table_col($('#product_sell_report_table'), 'total_depot_price'), true)
                );
