...
<div class="row">
        ...
        @if( $contact->type == 'supplier' || $contact->type == 'both')
            ...
            <div class="col-sm-12">
                @if(($contact->total_purchase - $contact->purchase_paid) > 0)
                    <a href="{{action([\App\Http\Controllers\TransactionPaymentController::class, 'getPayContactDue'], [$contact->id])}}?type=purchase" class="tw-dw-btn tw-dw-btn-primary tw-text-white tw-dw-btn-sm pull-right tw-m-2"><i class="fas fa-edit" aria-hidden="true"></i> @lang("contact.pay_due_amount")</a>
                @endif
            </div>
        @endif
   ...
   class="edit_contact_button tw-dw-btn tw-dw-btn-primary tw-text-white tw-dw-btn-sm pull-right tw-m-2">
    <i class="fas fa-edit" aria-hidden="true"></i>
    <span class="ml-2">@lang('messages.edit')</span>
</a>
    @endif

    <button type="button" class="tw-dw-btn tw-dw-btn-primary tw-text-white tw-dw-btn-sm pull-right tw-m-2" data-toggle="modal" data-target="#add_discount_modal">
