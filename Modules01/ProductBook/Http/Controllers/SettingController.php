...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/ProductBook/Resources/lang/kh/lang.php');
    }

    public function showProductBookPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 706)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('productbook::ProductBook.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('productbook::ProductBook.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $ProductBook = ProductBookSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('productbook::ProductBook.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = ProductBookSocial::firstOrNew([
