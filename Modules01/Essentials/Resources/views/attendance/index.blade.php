                    ...
                    <div class="tab-pane @if(!auth()->user()->can('essentials.crud_all_attendance')) active @endif" id="attendance_tab">
                        <div class="row">
                            @can('essentials.crud_all_attendance')
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        {!! Form::label('location_id', __('essentials::lang.location') . ':') !!}
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', '#employee_id, #date_range, #location_id', function() {
                attendance_table.ajax.reload();
            });
        ...
        function get_attendance_summary() {
            ...
            var location_id = $('#location_id').length ? $('#location_id').val() : '';
            
            var start = $('#date_range').data('daterangepicker').startDate.format('YYYY-MM-DD');
            var end = $('#date_range').data('daterangepicker').endDate.format('YYYY-MM-DD');
            $.ajax({
                url: '{{action([\Modules\Essentials\Http\Controllers\AttendanceController::class, 'getUserAttendanceSummary'])}}?user_id=' + user_id + '&location_id=' + location_id + '&start_date=' + start + '&end_date=' + end ,
                dataType: 'html',
