...
class EssentialsAllowanceAndDeductionController extends Controller
{
    ...
    public function index()
     {
         ...
         if (request()->ajax()) {
             ...
             if (request()->has('user_id') && !empty(request()->user_id)) {
                 $allowances->whereHas('employees', function ($query) {
            ...
            //  }
             ...
             if (request()->has('month_year') && !empty(request()->month_year)) {
                 $monthYear = request()->month_year; // Format: MM-YYYY
                 $date = \Carbon\Carbon::createFromFormat('m-Y', $monthYear);
                 $allowances->whereMonth('applicable_date', $date->month)
                            ->whereYear('applicable_date', $date->year);
             }
     
             if (request()->has('start_date') && !empty(request()->start_date) && request()->has('end_date') && !empty(request()->end_date)) {
                 $allowances->whereBetween('applicable_date', [request()->start_date, request()->end_date]);
             }
             ...
             return Datatables::of($allowances)
                 ->addColumn('action', function ($row) {
                     ...
                     if (auth()->user()->can('essentials.add_allowance_and_deduction')) {
                         ...
                         $html .= '<button data-href="' . action([\Modules\Essentials\Http\Controllers\EssentialsAllowanceAndDeductionController::class, 'view'], [$row->id]) . '" data-container="#add_allowance_deduction_modal" class="btn-modal tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-success"><i class="fa fa-eye" aria-hidden="true"></i> ' . __('messages.view') . '</button>';
                     }
                     return $html;
                 })
    ...
    public function view($id)
    {
        ...
        $selected_user = $allowance->employees->first();  // Get the first employee
        return view('essentials::allowance_deduction.view', compact('allowance', 'users', 'selected_user', 'applicable_date'));
        // return view('essentials::allowance_deduction.view')
