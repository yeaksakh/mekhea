...
use Illuminate\Support\Facades\Http; // Make sure this line is present
...
class EssentialsUtil extends Util
{
    ...
    public function clockin($data, $essentials_settings)
    {
        ...
        // NEW: Check if check-in time limit is set and if current time is past the limit
        if (!empty($essentials_settings['checkin_time'])) {
            $currentTime = \Carbon::now();
            $limitTime = \Carbon::createFromFormat('H:i', $essentials_settings['checkin_time']);

            // Check if current time is after the allowed check-in time
            if ($currentTime->format('H:i') > $limitTime->format('H:i')) {
                $output =  [
                    'success' => false,
                    'msg' => __('អ្នកមិនអាច check in ចាប់ពីម៉ោង :time', ['time' => $essentials_settings['checkin_time']])
        ...
        if ($count == 0) {
            ...
            $settings = json_decode($business->essentials_settings, true); // Decode JSON to array

            if (!empty($settings['telegram_alert_enabled'])) {
                $user = $this->getStuffName($data['user_id']);
                $checkInCount = $this->getTodayCheckInCount($data['business_id']);
                $role = $this->getCleanRoleName($data['user_id'], $data['business_id']);
                $this->checkIn($user->full_name, $role,  $settings, $checkInCount);
            }
        } else {
            $output = [
                'success' => false,
    ...
    public function clockout($data, $essentials_settings)
    {
        ...
        if (! empty($clock_in)) {
            ...
            if (! empty($settings['telegram_alert_enabled'])) {
                $user = $this->getStuffName($data['user_id']);
                $checkOutCount = $this->getTodayCheckOutCount($data['business_id']);
                $role = $this->getCleanRoleName($data['user_id'], $data['business_id']);
                $this->checkOut($user->full_name, $role,  $settings, $checkOutCount);
            }
        } else {
            $output = [
                'success' => false,
    ...
    public function getPayrollQuery($business_id)
    {
        $payrolls = Transaction::where('transactions.business_id', $business_id)
            ...
            ->select([
                ...
                'transactions.essentials_duration'
            ]);
    ...
    public function checkIn($name, $role, $setting, $count)
    {
        $this->sendTelegramNotification($name, $role, 'Check-In', '🟢', $setting, $count);
    }

    /**
     * Handles the check-out logic and sends a notification.
     */
    public function checkOut($name, $role, $setting, $count)
    {
        $this->sendTelegramNotification($name, $role, 'Check-Out', '🔴', $setting, $count);
    }
     ...
     * @param string $event The event type (e.g., 'Check-In', 'Check-Out').
     * @param string $emoji The emoji to use for the event (e.g., '🟢', '🔴').
     * @return void
     */

    private function sendTelegramNotification(string $staffName, string $role, string $event, string $emoji, $setting, $count)
    {
        ...
        $khmerMeridians = ['AM' => 'ព្រឹក', 'PM' => 'ល្ងាច'];

        $khmerDay = $khmerDays[$englishDay] ?? 'N/A';
        $khmerMeridian = $khmerMeridians[$englishMeridian] ?? '';
        $timePart = now()->format('h:i');
        $formattedTime = "{$khmerDay}, {$timePart} {$khmerMeridian}";

        // --- Message Preparation ---
        $message = "{$emoji} <b>{$event} ទី {$count}</b>\n";
        $message .= "<b>ឈ្មោះ:</b> {$staffName}\n";
        $message .= "<b>ម៉ោង:</b> {$formattedTime}\n";
        $message .= "<b>តួនាទី:</b> {$role}\n";
        ...
        if ($response->successful()) {
            Log::info("✅ {$event} notification sent", $logData);
        } else {
            Log::error("❌ Failed to send {$event} notification", array_merge($logData, [
                'error' => $response->json(),
    ...
    private function getStuffName($stuff_id)
    {
        return $user = User::where('id', $stuff_id)
            ->select([
                'username',
    ...
    private function getTodayCheckInCount($business_id)
    {
        return EssentialsAttendance::where('business_id', $business_id)
            ->whereDate('clock_in_time', \Carbon::today())
            ->whereNotNull('clock_in_time')
            ->count();
    }

    private function getTodayCheckOutCount($business_id)
    {
        return EssentialsAttendance::where('business_id', $business_id)
            ->whereDate('clock_out_time', \Carbon::today())
            ->whereNotNull('clock_out_time')
            ->count();
    }

    private function getCleanRoleName(int $user_id, int $business_id): string
    {
        $role = optional(DB::table('model_has_roles')
        ...
        return $role ? str_replace('#' . $business_id, '', $role) : '';
    }
}
