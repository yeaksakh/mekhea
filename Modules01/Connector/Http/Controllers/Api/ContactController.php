...
class ContactController extends ApiController
{
    ...
    //         $has_permission = true;
    //     } else {
    //         // Check for permissions based on the structure provided
    //         if (
    //             $user->can('supplier.view') || $user->can('customer.view') ||
    //             $user->can('supplier.view_own') || $user->can('customer.view_own')
    //         ) {
    //             $has_permission = true;
    //         }
    //     }
    ...
    //     // We still need to join the user_contact_access table for filtering,
    //     // but we won't select individual user data in the main query
    //     $query->leftJoin('user_contact_access AS uca', 'contacts.id', '=', 'uca.contact_id');

    //     // Filter based on user permissions and role
    //     if (!$is_admin) {
    //         // If user can only view own data (not all data)
    //         $view_own_contact = $user->can('supplier.view_own') || $user->can('customer.view_own');
    //         $view_all_contact = $user->can('supplier.view') || $user->can('customer.view');

    //         // If user can only view own data and not all data
    //         if ($view_own_contact && !$view_all_contact) {
    //             $query->where(function ($q) use ($user) {
    //                 $q->where('contacts.created_by', $user->id)
    //                     ->orWhere('uca.user_id', $user->id);
    //             });
    //         }
    //     }

    //     // Filter by assigned user (keep this filter since we're still joining uca)
    ...
    //             }
    //             if (!empty($search['biz_name'])) {
    //                 $query->orWhere('contacts.supplier_business_name', 'like', '%' . $search['name'] . '%');
    //             }
    ...
    //     // Apply having filters after groupBy
    ...
    //     // For each contact, fetch all assigned users in a separate query
    //     foreach ($contacts as $contact) {
    //         $assignedUsers = DB::table('user_contact_access AS uca')
    //             ->leftJoin('users AS u', 'uca.user_id', '=', 'u.id')
    //             ->where('uca.contact_id', $contact->id)
    //             ->select(
    ...
    //         // Add the assigned users array to the contact
    //         $contact->assigned_to_users = $assignedUsers;
    //     }
   ...
   public function index()
{
    ...
    // Apply ordering - Show latest first by default
    $order_by = request()->input('order_by');
    $order_by_dir = request()->input('direction');
    if (!empty($order_by) && !empty($order_by_dir)) {
        $query->orderBy($order_by, $order_by_dir);
    } else {
        // Default ordering by created_at descending (latest first)
    ...
    // Validate per_page to prevent excessive loads
    if ($per_page > 100) {
        $per_page = 100;
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            DB::beginTransaction();
             if ($request->input('is_membership') == '1') {
                $input['member'] = 1;
            }else{
                $input['member'] = 0;
            }
    ...
    public function show($contact_ids)
{
    ...
    $contacts = $query
        ...
        ->leftJoin('media', function ($join) {
            $join->on('contacts.id', '=', 'media.model_id')
                 ->where('media.model_media_type', '=', 'profile_photo');
        })
        ->addSelect(
            ...
            'media.file_name AS profile_photo'
        )
        ->groupBy('contacts.id')
        ->get();
    ...
    foreach ($contacts as $key => $value) {
        ...
        if ($value->profile_photo) {
            $contacts[$key]->profile_photo_url = url('uploads/media/' . $value->profile_photo);
        }
    }
