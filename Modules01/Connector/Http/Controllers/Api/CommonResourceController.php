...
class CommonResourceController extends ApiController
{
    ...
    public function getNotifications()
    {
        ...
        $perPage = request()->input('per_page', $this->perPage); // Use $this->perPage as default if not provided
        ...
        if ($perPage === '-1') {
            ...
            $notifications->appends(request()->query()); // Append other query parameters to pagination links
        }
    ...
    public function updateNotification()
    {
        ...
        // Retrieve the notifications that are unread (read_at is null)
        $notifications = $user->notifications()
            ->whereNull('read_at')
            ->orderBy('created_at', 'DESC')
            ->get();

        // Update the read_at timestamp to the current time for each unread notification
        ...
        // Optionally, parse the updated notifications
        $notifications_data = $this->commonUtil->parseNotifications($notifications);
