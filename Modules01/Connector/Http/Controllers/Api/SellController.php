...
class SellController extends ApiController
{
    ...
    public function index()
    {
        ...
        if (!empty($filters['status'])) {
            ...
            } elseif ($filters['status'] == 'consignment') {
                $query->where('transactions.sub_status', 'consignment')
                    ->where('transactions.is_consignment', 1);
        ...
        if (!empty($filters['commission_agent'])) {
            $query->where('transactions.commission_agent', $filters['commission_agent']);
        }
  ...
  public function show($sell_ids)
    {
        ...
        // Relationships to load, excluding 'activities'
        $with = [
            'sell_lines',
            'payment_lines',
            'sell_lines.product',
        ...
        // Retrieve transactions with specified relationships
        $sells = $query->with($with)->get();

        // Attach activities manually to each transaction
        foreach ($sells as $sell) {
            // Assuming Activity model and the forSubject method work with the Transaction model
            $sell->activities = Activity::forSubject($sell)
                ->with(['causer', 'subject'])
                ->latest()
                ->get();
        }

        // Return the transactions as a resource collection with activities included
        ...
        try {
            ...
            foreach ($sells as $sell_data) {
                try {
                    ...
                    $input['is_membership'] = $request->is_membership;
                    ...
                    $change_return['method'] = $request->method;
                    ...
                    if ($input['status'] == 'final') {
                        ...
                        if ($request->is_membership == 1) {
                            // Find the ContactPackage for the given contact_id
                            $contactPackage = ContactPackage::where('contact_id', $input['contact_id'])
                                ->where('business_id', $input['business_id'])
                                ->first();

                            if ($contactPackage > 0) {

                                $new_package_qty = $contactPackage->package_qty - $pack_qty;
                                $contactPackage->update(['package_qty' => $new_package_qty]);
                            } else {
                                // Create a new ContactPackage if none exists
                                ContactPackage::create([
                                    'contact_id' => $input['contact_id'],
                                    'business_id' => $input['business_id'],
                                    'package_id' => $input['package_id'] ?? null, // Assuming package_id is provided or nullable
    ...
    public function storeTransactionPayment(Request $request)
{
    try {
        ...
        // Additional data preparation
        $inputs['transaction_id'] = $transaction->id;
        $inputs['amount'] = $this->transactionUtil->num_uf($inputs['amount'] ?? 0);
        $inputs['created_by'] = auth()->user()->id;
        $inputs['payment_for'] = $transaction->contact_id;
        ...
        if ($request->has('id') && !empty($request->id)) {
            $existingPayment = TransactionPayment::where('id', $request->id)
                ->where('business_id', $business_id)
                ->first();
        }
        
        // If no ID provided or not found by ID, check by transaction_id to prevent duplicates
        if (!$existingPayment) {
            $existingPayment = TransactionPayment::where('transaction_id', $transaction_id)
                ->where('business_id', $business_id)
                ->first();
        }
        ...
        if ($existingPayment) {
            ...
            $existingPayment->fill(array_filter($inputs, function ($value) {
            ...
            // Log the updated transaction payment
            \Log::info('TransactionPayment updated successfully', ['transaction_payment' => $existingPayment]);
            ...
            return response()->json([
                ...
                'transaction_payment' => $existingPayment
            ]);
        }
        ...
        // Create and save new transaction payment
        $tp = new TransactionPayment($inputs);
        $tp->save();
        ...
        // Log any errors
        \Log::error('Error saving or updating TransactionPayment:', ['error' => $e->getMessage()]);

        // Return error response
        return response()->json([
            'success' => false,
            'msg' => 'Failed to add or update payment.'
        ], 500);
    }
} 
    ...
    public function updateTransectionPayment(Request $request, $id)
    {
        // Authorization check
        if (
        ...
        $validator = Validator::make($request->all(), [
            ...
            'card_month' => 'nullable|integer|min:1|max:12',
        ...
        try {
            ...
            // Check if payment belongs to user's business
            $transaction = Transaction::where('business_id', $business_id)
            ...
            return response()->json([
                'success' => true,
                'message' => __('purchase.payment_updated_success'),
                'data' => [
...
{
    // Authorization check
    if (!auth()->user()->can('delete_purchase_payment') && 
    ...
    try {
        // Get business_id from authenticated user for security
        $business_id = auth()->user()->business_id;

        $payment = TransactionPayment::findOrFail($id);

        // Verify payment belongs to user's business (if transaction exists)
        ...
        if (!empty($payment->transaction_id)) {
            ...
            $message = __('purchase.payment_deleted_success');
            ...
            if ($total_customer_advance > 0) {
                $this->transactionUtil->updateContactBalance(
                    $payment->payment_for, 
                    $total_customer_advance, 
                    'deduct'
                );
            }

            $deleted_child_payments = [];
            
            // Delete all child payments
            foreach ($adjusted_payments as $adjusted_payment) {
                $deleted_child_payments[] = [
                    'id' => $adjusted_payment->id,
                    'amount' => $adjusted_payment->amount,
                    'transaction_id' => $adjusted_payment->transaction_id
                ];
            ...
            $deleted_payment_info['deleted_child_payments'] = $deleted_child_payments;
            $deleted_payment_info['customer_advance_deducted'] = $total_customer_advance;
            
            $message = __('purchase.advance_payment_deleted_success');
        }
        ...
        return response()->json([
            ...
            'message' => $message,
            'data' => [
                'deleted_payment' => $deleted_payment_info
            ]
        ], 200);
        ...
        \Log::emergency('Payment Delete Error - File:' . $e->getFile() . ' Line:' . $e->getLine() . ' Message:' . $e->getMessage());
