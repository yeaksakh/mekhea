    ...
    public function storeTransactionPayment(Request $request)
{
    try {
        // Prepare the necessary data
        $transaction_id = $request->input('transaction_id');
        $payment_id = $request->input('payment_id'); // To identify if updating an existing payment
        $user = Auth::user();
        $business_id = $user->business_id;  
        ...
        if ($payment_id) {
            // Find the existing transaction payment
            $tp = TransactionPayment::where('business_id', $business_id)->findOrFail($payment_id);
            $tp->update($inputs);
            $action = 'updated';
        } else {
        ...
        // Log the saved or updated transaction payment
        \Log::info("TransactionPayment {$action} and payment status updated successfully", ['transaction_payment' => $tp]);

        // Return success response
        return response()->json([
            'success' => true,
            'msg' => "Payment {$action} successfully and payment status updated!",
            'transaction_payment' => $tp
        ]);
    } catch (\Exception $e) {
        // Rollback the transaction in case of error
        ...
        // Log any errors
        \Log::error('Error saving or updating TransactionPayment or updating payment status:', ['error' => $e->getMessage()]);
        
        // Return error response
        return response()->json([
            'success' => false,
            'msg' => 'Failed to add or update payment or update payment status.'
        ], 500);
    }
}
