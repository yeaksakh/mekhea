...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/News/Resources/lang/kh/lang.php');
    }

    public function showNewsPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 883)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('news::News.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('news::News.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $News = NewsSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('news::News.setting', [
            'roles' => $roles,
