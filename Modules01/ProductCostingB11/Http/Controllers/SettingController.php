...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/ProductCostingB11/Resources/lang/kh/lang.php');
    }

    public function showProductCostingB11PermissionForm()
    {
        ...
        $permissions = Permission::where('id', 575)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('productcostingb11::ProductCostingB11.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('productcostingb11::ProductCostingB11.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('productcostingb11::ProductCostingB11.setting', [
            'roles' => $roles,
