@extends('layouts.app')
@section('title', 'Employee Work Tracking Report')
...
@section('content')
    ...
    <div style="margin: 16px" class="no-print">
        @component('components.filters', ['title' => 'Employee Work Tracking Filters'])
            <div class="col-md-3">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="department_filter" name="department_filter">
                        ...
                        @foreach($departments as $id => $name)
                            <option value="{{ $id }}" {{ $department_id == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-3">
             ...
             @include('employeetracker::components.printbutton', [
                'report_name' =>  __('minireportb1::minireportb1.mony-003'),
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // Date range picker (auto apply like income_for_month)
            ...
            var employee_work_table = $('#employee_work_table').DataTable({
                ...
                serverSide: false,
                ajax: {
                    url: "{{ action([\Modules\EmployeeTracker\Http\Controllers\EmployeeReportController::class, 'getEmployeeWorkTrackingReport']) }}",
                    data: function(d) {
                ...
                columns: [
                    ...
                    { data: 'name', name: 'name' },
                    { data: 'total_transactions', name: 'total_transactions', className: 'text-center' },
                    { data: 'expense_count', name: 'expense_count', className: 'text-center' },
                    { data: 'purchase_count', name: 'purchase_count', className: 'text-center' },
                    { data: 'sell_count', name: 'sell_count', className: 'text-center' },
                    { 
                        data: 'work_status', 
                        name: 'work_status',
                        render: function(data) {
                            var statusClass = data === 'Working' ? 'status-working' : 'status-not-working';
                            var icon = data === 'Working' ? '✓' : '✗';
                            return '<span class="' + statusClass + '">' + icon + ' ' + data + '</span>';
                        }
                    },
                ...
                buttons: [
                    { extend: 'colvis', text: 'Columns <i class="fas fa-caret-down"></i>' },
                    { extend: 'csv', text: 'CSV <i class="fa fa-file-csv"></i>' },
                    { extend: 'excel', text: 'Excel <i class="fa fa-file-excel"></i>' },
                    { extend: 'pdf', text: 'PDF <i class="fa fa-file-pdf"></i>' }
                ],
                footerCallback: function(row, data, start, end, display) {
                    var api = this.api();
                    
                    var totalTransactions = api.column(3).data().reduce((a, b) => a + parseInt(b || 0), 0);
            ...
            // Buttons removed (auto-filter via date range)
