...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/EmployeeTracker/Resources/lang/kh/lang.php');
    }

    public function showEmployeeTrackerPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 890)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('employeetracker::EmployeeTracker.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('employeetracker::EmployeeTracker.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $EmployeeTracker = EmployeeTrackerSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('employeetracker::EmployeeTracker.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = EmployeeTrackerSocial::firstOrNew([
