...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/Announcement/Resources/lang/kh/lang.php');
    }

    public function showAnnouncementPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 676)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('announcement::Announcement.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('announcement::Announcement.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $Announcement = AnnouncementSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('announcement::Announcement.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = AnnouncementSocial::firstOrNew([
