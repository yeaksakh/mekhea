...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/CustomerStock/Resources/lang/kh/lang.php');
    }

    public function showCustomerStockPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 896)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('customerstock::CustomerStock.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('customerstock::CustomerStock.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $CustomerStock = CustomerStockSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('customerstock::CustomerStock.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = CustomerStockSocial::firstOrNew([
