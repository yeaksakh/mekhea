...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/BotTelegramManager/Resources/lang/kh/lang.php');
    }

    public function showBotTelegramManagerPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 905)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('bottelegrammanager::BotTelegramManager.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('bottelegrammanager::BotTelegramManager.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $BotTelegramManager = BotTelegramManagerSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('bottelegrammanager::BotTelegramManager.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = BotTelegramManagerSocial::firstOrNew([
