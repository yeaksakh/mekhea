...
class IndexCategoryController extends Controller
{
    ...
    public function createIndexCategory($moduleName)
    {
        // Your function implementation here
        // Example content creation code
        $viewPath = base_path("Modules/{$moduleName}/Resources/views/Category/index.blade.php");
        ...
        if (!$this->files->exists($viewPath)) {
            ...
            @section('content')
                ...
                <section class="content-header no-print">
                    <h1>
                        @lang('{$moduleNameLower}::lang.category')
                    </h1>
                </section>
                <!-- Main content -->
                <section class="content no-print">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            <h5 class="box-title">
                                @lang('{$moduleNameLower}::lang.all_category')
                            </h5>
                            <div class="box-tools pull-right">
                                <button type="button" class="btn btn-sm btn-primary btn-modal" data-href="{{ route('{$moduleName}-categories.create') }}" data-container=".category_modal">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered table-striped" id="categories_table">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>@lang('{$moduleNameLower}::lang.name')</th>
                                            <th>@lang('{$moduleNameLower}::lang.image')</th>
                                            <th>@lang('{$moduleNameLower}::lang.description')</th>
                ...
                <script type="text/javascript">
                    $(document).ready(function() {
                        var table = $('#categories_table').DataTable({
                            ...
                            serverSide: true,
                            ajax: {
                                url: "{{ action([\\Modules\\{$moduleName}\\Http\\Controllers\\{$moduleName}Controller::class, 'getCategories']) }}",
                            },
                            order: [[1, 'desc']],
                            columns: [
                                {
