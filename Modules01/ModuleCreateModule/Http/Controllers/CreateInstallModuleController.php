...
class CreateInstallModuleController extends Controller
{
    protected $files;
    ...
    /**
     * Dynamically creates the InstallController for the given module.
     *
     * @param string $moduleName
     * @return void
     */
    public function createInstallController($moduleName)
    {
        $controllerPath = base_path("Modules/{$moduleName}/Http/Controllers/InstallController.php");

        if (!$this->files->exists($controllerPath)) {

            $moduleNameLowercase = strtolower($moduleName);
            $appVersion = config("{$moduleName}.module_version");
            $moduleDisplayName = ucfirst($moduleName);

            // Dynamically generate the content for the InstallController class
            $content = <<<EOT
            <?php
            ...
            class InstallController extends Controller
            {
                ...
                {
                    \$this->module_name = '{$moduleNameLowercase}';
                    \$this->appVersion = config('{$moduleNameLowercase}.module_version');
                    \$this->module_display_name = '{$moduleDisplayName}';
                }
                ...
                public function index()
                {
                    ...
                    if (empty(\$is_installed)) {
                        ...
                        Artisan::call('module:migrate', ['module' => '{$moduleDisplayName}', '--force' => true]);
                        System::addProperty(\$this->module_name . '_version', \$this->appVersion);
                    }

                    \$action_url = action([\\Modules\\{$moduleName}\\Http\\Controllers\\InstallController::class, 'install']);
                    \$intruction_type = 'uf';
                    \$action_type = 'install';
                ...
                public function update()
                {
                    ...
                    try {
                        ...
                        \$output = [
                            'success' => 1,
                            'msg' => '{$moduleDisplayName} module updated successfully to version ' . \$this->appVersion . ' !!',
                        ];
                ...
                public function install()
                {
                    try {
                        ...
                        \$output = ['success' => 1, 'msg' => '{$moduleDisplayName} module installed successfully'];
                    } catch (\\Exception \$e) {
            ...
            // Save the generated content to the InstallController file
