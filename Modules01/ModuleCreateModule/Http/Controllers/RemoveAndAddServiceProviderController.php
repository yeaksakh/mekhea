...
class RemoveAndAddServiceProviderController extends Controller
{
    protected $files;
    ...
    public function removeAndAddServiceProvider($moduleName)
    {
        $routesPath = base_path("Modules/{$moduleName}/Providers/{$moduleName}ServiceProvider.php");
        $moduleNameLower = strtolower($moduleName);

        Log::info("Routes path: {$routesPath}");

        // Define the new service provider content
        $newContent = <<<EOT
        <?php
        ...
        class {$moduleName}ServiceProvider extends ServiceProvider
        {
            ...
            protected string \$moduleNameLower = '{$moduleNameLower}';

            protected \$middleware = [
                '{$moduleName}' => [
                    '{$moduleName}Language' => 'ModuleLanguageMiddleware',
                ],
            ];
        ...
        // Save the new content to the service provider file
        try {
            $this->files->put($routesPath, $newContent);
            Log::info("Routes file created: {$routesPath}");
        } catch (\Exception $e) {
            Log::error("Failed to create the routes file at {$routesPath}: " . $e->getMessage());
        }
    }
}