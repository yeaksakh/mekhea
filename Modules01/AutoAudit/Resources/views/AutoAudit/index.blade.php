    ...
    <section class="content no-print">
        <nav
            ...
            <div class="container-fluid">
                ...
                <li @if (request()->segment(1) == 'sells' && request()->segment(2) == null) class="active" @endif>
                    <a
                        href="{{ action([\Modules\AutoAudit\Http\Controllers\AuditController::class, 'index']) }}">{{ 'Invoice For Audit' }}</a>
                </li>
        ...
        @component('components.filters', ['title' => __('report.filters')])
            ...
            <div class="col-md-3">
                <div class="form-group">
                    <br>
                    <button type="button" class="tw-dw-btn tw-dw-btn-primary" id="auto_audit_button">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                            ...
                            <path d="M7.5 3.225c1.472 -.658 3.085 -1 4.5 -1" />
        ...
        @component('components.widget', ['class' => 'box-primary', 'title' => __('lang_v1.all_sales')])
            @can('direct_sell.access')
                @slot('tool')
                    <div class="box-tools">
                        <a class="tw-dw-btn tw-font-bold tw-text-white tw-border-none tw-rounded-full pull-right tw-m-2 tw-mb-4"
                            style="background: linear-gradient(to right, @if (!empty(session('business.theme_color'))) {{ session('business.theme_color') === 'primary' ? '#1e40af' : (session('business.theme_color') === 'green' ? '#15803d' : '#1e40af') }} @else #1e40af @endif, @if (!empty(session('business.theme_color'))) {{ session('business.theme_color') === 'primary' ? '#1e3a8a' : (session('business.theme_color') === 'green' ? '#166534' : '#1e3a8a') }} @else #1e3a8a @endif); padding: 0.5rem 1rem; font-size: 0.875rem;"
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#only_francier').iCheck({
                checkboxClass: 'icheckbox_square-blue' // Adjust based on your iCheck theme
            });
            ...
            sell_table = $('#sell_table').DataTable({
                ...
                "ajax": {
                    "url": "{{ action([\Modules\AutoAudit\Http\Controllers\AuditController::class, 'index']) }}",
                    "data": function(d) {
            ...
            $('#only_francier').on('ifChanged', function(event) {
                sell_table.ajax.reload();
            });

            $(document).on('click', '#auto_audit_button', function(e) {
                e.preventDefault();

                var button = $(this);
                button.prop('disabled', true).html('<i class="fa fa-spinner fa-spin"></i> Auditing...');

                $.ajax({
                    method: 'GET',
                    url: "{{ action([\Modules\AutoAudit\Http\Controllers\AuditController::class, 'autoAudit']) }}",
                    dataType: 'json',
                    success: function(result) {
                        if (result.success) {
                            toastr.success(result.message || 'Audit completed successfully!');
                            sell_table.ajax.reload();
                        } else {
