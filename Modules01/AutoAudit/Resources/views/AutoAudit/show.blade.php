    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('autoaudit::lang.details')</h4>
            ...
            <div class="modal-body" id="print-content">
                ...
                @if($autoaudit->category)
                    @php
                        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
                        $fileExtension = strtolower(pathinfo($autoaudit->category->image, PATHINFO_EXTENSION));
                    @endphp
                    
                    @if(in_array($fileExtension, $imageExtensions))
                        <label class="form-check-label pt-2 mb-0" for="categoryCheck">
                            <img src="{{ asset('uploads/AutoAuditCategory/' . basename($autoaudit->category->image)) }}" 
                                alt="Document Image" 
                                style="max-width: 25px; max-height: 25px; vertical-align: middle;">
                            </label>
                    @elseif($fileExtension === 'pdf')
                        <span class="me-2">
                            <a href="{{ asset('uploads/AutoAuditCategory/' . basename($autoaudit->category->image)) }}" 
                            target="_blank" 
                    ...
                    @endif
                    <span>{{ $autoaudit->category->name }}</span>
                    <div class="form-group" id="category-detail">
                        <label for="categorydetail">@lang('employeecontractb1::lang.description'):</label>
                        <p id="categorydetail" class="form-control-static">{!! $autoaudit->category->description !!}</p>
                    </div>
                    @endif
                </label>
                <!-- Modal Content Goes Here -->
                    <div class="form-group" id="transaction_idcontent">
        <label for="transaction_id">@lang('autoaudit::lang.transaction_id'):</label>
        <p id="transaction_id" class="form-control-static">{{ $autoaudit->{'transaction_id'} }}</p>
    </div>
    <div class="form-group" id="audit_status_2content">
        <label for="audit_status_2">@lang('autoaudit::lang.audit_status_2'):</label>
        <p id="audit_status_2" class="form-control-static">{{ $autoaudit->{'audit_status_2'} }}</p>
    </div>
                ...
                <div class="form-group" id="createdatcontent">
                    ...
                    <p id="createdat" class="form-control-static">{{ $autoaudit->created_at }}</p>
                </div>
                <div class="form-group" id="qrcontent">
                    <label for="qrcode">@lang('autoaudit::lang.qrcode'):</label>
