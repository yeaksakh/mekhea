...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/AiStudio/Resources/lang/kh/lang.php');
    }

    public function showAiStudioPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 703)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('aistudio::AiStudio.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('aistudio::AiStudio.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $AiStudio = AiStudioSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('aistudio::AiStudio.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = AiStudioSocial::firstOrNew([
