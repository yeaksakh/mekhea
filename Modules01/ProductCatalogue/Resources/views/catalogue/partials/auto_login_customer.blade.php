        ...
        <div class="col-md-7">
            @component('components.widget', ['class' => 'box-solid'])
                <div class="form-group">
                    {!! Form::label('cust_id', __('productcatalogue::lang.customer') . ':') !!}
                    {!! Form::select('cust_id', $user_customer, null, ['class' => 'form-control select2', 'id' => 'cust_id', 'placeholder' => __('messages.please_select'),  'style' => 'width: 700px' ]) !!}
                </div>
                <div class="form-group">
                ...
                <button type="button" class="btn btn-primary" id="generate_qr_cust">@lang('productcatalogue::lang.generate_qr')</button>
            @endcomponent
            @component('components.widget', ['class' => 'box-solid'])
                <div class="row">
                    <div class="col-md-12">
        ...
        <div class="col-md-5">
            @component('components.widget', ['class' => 'box-solid'])
                <div class="text-center">
                    <div id="cust_qrcode"></div>
                    <span id="cust_link"></span>
                    <br>
                    <a href="#" class="btn btn-success hide" id="cust_download_image">@lang('productcatalogue::lang.download_image')</a>
                </div>
            @endcomponent
        </div>
    </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        (function($) {
            ...
            $(document).on('click', '#generate_qr_cust', function(e) {
                $('#cust_qrcode').html('');
                if ($('#cust_id').val()) {
                    // Get the selected customer ID and username from the select element
                    var customerId = $('#cust_id').val();
                    var username = $('#cust_id option:selected').text().trim();
                    ...
                    new QRCode(document.getElementById("cust_qrcode"), custOpts);
                    $('#cust_link').html('<a target="_blank" href="' + custLink + '">Customer Link</a>');
                    $('#cust_download_image').removeClass('hide');
                    $('#cust_qrcode').find('canvas').attr('id', 'cust_qr_canvas');
                } else {
                    alert("{{ __('lang_v1.please_select_customer') }}");
                }
            });

            $('#cust_download_image').click(function(e) {
                e.preventDefault();
