        ...
        <div class="col-12 col-md-8 col-lg-6 mt-4">
            <div class="card">
                ...
                <div class="card-block">
                    ...
                    <!-- Price Display -->
                    @if($group_price_id)
                        @if(isset($has_selected_group_price) && $has_selected_group_price && !empty($selected_group_price_details))
                            <!-- Show specific selected group price -->
                            <div class="mt-2">
                                @foreach($selected_group_price_details as $variation_id => $price_info)
                                    <p>
                                        <strong>Price:</strong> 
                                        <span class="display_currency" data-currency_symbol="true">{{ number_format($price_info['price'], 2) }}</span>
                                        <small class="text-muted">({{ $allowed_group_prices[$group_price_id] ?? 'Group Price' }})</small>
                                    </p>
                                @endforeach
                            </div>
                        @else
                            <!-- Show message when no price exists for the selected group -->
                            <div class="mt-2">
                                <p>
                                    <strong>Price:</strong> 
                                    <span class="text-danger">No price available for {{ $allowed_group_prices[$group_price_id] ?? 'this group' }}</span>
                                </p>
                            </div>
                        @endif
                    @else
                        <!-- Show default selling price when no group price is selected -->
                        @foreach ($product->variations as $variation)
                            <p>
                                <strong>Default Price:</strong> 
                                <span class="display_currency" data-currency_symbol="true">{{ number_format($variation->default_sell_price, 2) }}</span>
                                <small class="text-muted">(1.ទូទៅ)</small>
                            </p>
                        @endforeach
                    @endif
                </div>
...
<script>
 $(document).ready(function() {
    ...
    // Upscale canvas to 1024×1024 for sharper ~1 MB file
    ...
    // Draw original QR scaled up
    ctx.drawImage(canvas, 0, 0, 1024, 1024);

    // Trigger download
    bigCanvas.toDataURL('image/png', 1.0).replace("image/png", "image/octet-stream");
    const link = document.createElement('a');
    link.download = 'product_qrcode.png';
    link.href = bigCanvas.toDataURL('image/png', 1.0);
    link.click();
});
});
</script>
