        ...
        <div class="col-md-7">
            @component('components.widget', ['class' => 'box-solid'])
                <div class="form-group">
                    {!! Form::label('item_id', __('productcatalogue::lang.product') . ':') !!} <!-- Unique 'item_id' for product selection -->
                    {!! Form::select('item_id', $products, null, [
                        'class' => 'form-control select2',
                        'id' => 'item_id',
                        'placeholder' => __('messages.please_select'),  'style' => 'width: 700px' 
                    ]) !!}
                </div>

                <div class="form-group">
                    {!! Form::label('group_price_id', __('productcatalogue::lang.price_group') . ':') !!} <!-- Unique 'item_id' for product selection -->
                    {!! Form::select('group_price_id', $group_prices, null, [
                        'class' => 'form-control select2',
                        'id' => 'group_price_id',
                        'placeholder' => __('messages.please_select'),  'style' => 'width: 700px' 
                    ]) !!}
                ...
                <div class="form-group">
                    ...
                    {!! Form::text('item_subtitle', __('productcatalogue::lang.product_catalogue'), [
                        'class' => 'form-control',
                        'id' => 'item_subtitle',
                    ]) !!}
                </div>
                ...
                <button type="button" class="btn btn-primary" id="generate_qr_item">@lang('productcatalogue::lang.generate_qr')</button>
                <!-- Unique 'generate_qr_item' -->
            @endcomponent
            @component('components.widget', ['class' => 'box-solid'])
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <table class="table table-striped">
                            <tr>
                                <td>1</td>
                                <td>@lang('productcatalogue::lang.catalogue_instruction_6')</td>
                            </tr>
                            <tr>
                                <td>2</td>
        ...
        <div class="col-md-5">
            @component('components.widget', ['class' => 'box-solid'])
                <div class="text-center">
                    ...
                    <br>
                    <a href="#" class="btn btn-success hide" id="item_download_image">@lang('productcatalogue::lang.download_image')</a>
                    <!-- Unique 'item_download_image' -->
                </div>
            @endcomponent
        </div>
    </div>
    ...
    <script type="text/javascript">
        (function($) {
            ...
            $(document).on('click', '#generate_qr_item', function(e) { // Unique 'generate_qr_item'
                ...
                var groupPriceId = $('#group_price_id').val(); // Get selected group price ID
                ...
                var businessId = "{{ $business_id }}"; // Pass the business_id to JavaScript
                
                // Construct the URL - if group price is selected, include it in the URL
                var itemLink;
                if (groupPriceId) {
                    itemLink = "{{ url('/product') }}/" + businessId + "/" + productId + "/" + groupPriceId;
                } else {
                    // If no group price selected, just use product ID (no group price parameter)
                    itemLink = "{{ url('/product') }}/" + businessId + "/" + productId;
                }
