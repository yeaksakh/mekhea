    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('backup::lang.details')</h4>
            ...
            <div class="modal-body" id="print-content">
                ...
                @if($backup->category)
                    @php
                        ...
                        $fileExtension = strtolower(pathinfo($backup->category->image, PATHINFO_EXTENSION));
                    @endphp
                    
                    @if(in_array($fileExtension, $imageExtensions))
                        <label class="form-check-label pt-2 mb-0" for="categoryCheck">
                            <img src="{{ asset('uploads/BackupCategory/' . basename($backup->category->image)) }}" 
                                alt="Document Image" 
                                style="max-width: 25px; max-height: 25px; vertical-align: middle;">
                            </label>
                    @elseif($fileExtension === 'pdf')
                        <span class="me-2">
                            <a href="{{ asset('uploads/BackupCategory/' . basename($backup->category->image)) }}" 
                            target="_blank" 
                    ...
                    @endif
                    <span>{{ $backup->category->name }}</span>
                    <div class="form-group" id="category-detail">
                        <label for="categorydetail">@lang('employeecontractb1::lang.description'):</label>
                        <p id="categorydetail" class="form-control-static">{!! $backup->category->description !!}</p>
                    </div>
                    @endif
                </label>
                <!-- Modal Content Goes Here -->
                    <div class="form-group" id="name_1content">
        <label for="name_1">@lang('backup::lang.name_1'):</label>
        <p id="name_1" class="form-control-static">{{ $backup->name1->first_name . ' ' . $backup->name1->last_name }}</p>
    </div>
                ...
                <div class="form-group" id="createdatcontent">
                    ...
                    <p id="createdat" class="form-control-static">{{ $backup->created_at }}</p>
                </div>
                <div class="form-group" id="qrcontent">
                    <label for="qrcode">@lang('backup::lang.qrcode'):</label>
