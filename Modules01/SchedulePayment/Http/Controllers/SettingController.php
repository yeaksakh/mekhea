...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/SchedulePayment/Resources/lang/kh/lang.php');
    }

    public function showSchedulePaymentPermissionForm()
    {
        ...
        $permissions = Permission::where('id', 697)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('schedulepayment::SchedulePayment.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('schedulepayment::SchedulePayment.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $SchedulePayment = SchedulePaymentSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('schedulepayment::SchedulePayment.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = SchedulePaymentSocial::firstOrNew([
