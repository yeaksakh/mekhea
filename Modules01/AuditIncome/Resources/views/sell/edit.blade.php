...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	{!! Form::open(['url' => action([\Modules\AuditIncome\Http\Controllers\SellPosController::class, 'update'], ['po' => $transaction->id ]), 'method' => 'put', 'id' => 'edit_sell_form', 'files' => true ]) !!}
			...
			@component('components.widget', ['class' => 'box-solid'])
				...
				@php
					if($transaction->status == 'draft' && $transaction->is_quotation == 1){
						...
						$status = 'proforma';
					}else if ($transaction->sub_status == 'consignment') {
						$status = 'consignment';
					}else {
	...
	<div class="row">
		...
		<div class="col-sm-12 text-center tw-mt-4">
			<button type="button" id="submit-sell" class="tw-dw-btn tw-dw-btn-primary tw-dw-btn-lg tw-text-white" style="background-color: #006400;">@lang('messages.save')</button>
			<button type="button" id="save-and-print" class="tw-dw-btn tw-dw-btn-success tw-dw-btn-lg tw-text-white"style="background-color: #007BFF;">@lang('lang_v1.save_and_print')</button>
		</div>
	</div>
	@if(in_array('subscription', $enabled_modules))
