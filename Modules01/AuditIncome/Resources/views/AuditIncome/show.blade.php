    <div class="modal-dialog modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">@lang('auditincome::lang.details')</h4>
            ...
            <div class="modal-body" id="print-content">
                ...
                @if($auditincome->category)
                    @php
                        $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
                        $fileExtension = strtolower(pathinfo($auditincome->category->image, PATHINFO_EXTENSION));
                    @endphp
                    
                    @if(in_array($fileExtension, $imageExtensions))
                        <label class="form-check-label pt-2 mb-0" for="categoryCheck">
                            <img src="{{ asset('uploads/AuditIncomeCategory/' . basename($auditincome->category->image)) }}" 
                                alt="Document Image" 
                                style="max-width: 25px; max-height: 25px; vertical-align: middle;">
                            </label>
                    @elseif($fileExtension === 'pdf')
                        <span class="me-2">
                            <a href="{{ asset('uploads/AuditIncomeCategory/' . basename($auditincome->category->image)) }}" 
                            target="_blank" 
                    ...
                    @endif
                    <span>{{ $auditincome->category->name }}</span>
                    <div class="form-group" id="category-detail">
                        <label for="categorydetail">@lang('employeecontractb1::lang.description'):</label>
                        <p id="categorydetail" class="form-control-static">{!! $auditincome->category->description !!}</p>
                    </div>
                    @endif
                </label>
                <!-- Modal Content Goes Here -->
                    <div class="form-group" id="IncomeSource_1content">
        <label for="IncomeSource_1">@lang('auditincome::lang.IncomeSource_1'):</label>
        <p id="IncomeSource_1" class="form-control-static">{{ $auditincome->{'IncomeSource_1'} }}</p>
    </div>
    <div class="form-group" id="Amount_2content">
        <label for="Amount_2">@lang('auditincome::lang.Amount_2'):</label>
        <p id="Amount_2" class="form-control-static">{{ $auditincome->{'Amount_2'} }}</p>
    </div>
                ...
                <div class="form-group" id="createdatcontent">
                    ...
                    <p id="createdat" class="form-control-static">{{ $auditincome->created_at }}</p>
                </div>
                <div class="form-group" id="qrcontent">
                    <label for="qrcode">@lang('auditincome::lang.qrcode'):</label>
