...
border-radius: 10%; /* Optional: Makes the border round (circular) */
...
<tr>
<td>
	<b class="tw-text-base md:tw-text-lg tw-font-bold">@lang('sale.item'):</b>&nbsp;
...
<td @if(!Gate::check('disable_discount') || auth()->user()->can('superadmin') || auth()->user()->can('admin')) class="" @else class="hide" @endif>
	<b class="tw-text-base md:tw-text-lg tw-font-bold">
		...
		@if($edit_discount)
		<i class="fas fa-trophy color-icon" id="pos-edit-discount" title="@lang('sale.edit_discount')" aria-hidden="true" data-toggle="modal" data-target="#posEditDiscountModal"></i>
...
<td class="@if($pos_settings['disable_order_tax'] != 0) hide @endif">
	<span class="tw-text-base md:tw-text-lg tw-font-semibold">
		...
		<i class="fas fa-building color-icon" title="@lang('sale.edit_order_tax')" aria-hidden="true" data-toggle="modal" data-target="#posEditOrderTaxModal" id="pos-edit-tax"></i>
...
<td>
	<span class="tw-text-base md:tw-text-lg tw-font-semibold">
		...
		<i class="fas fa-truck color-icon" title="@lang('sale.shipping')" aria-hidden="true" data-toggle="modal" data-target="#posShippingModal"></i>
...
<td>
	<i class="fas fa-boxes color-icon service_modal_btn"></i>
...
<td>
	<i class="fas fa-stop-circle color-icon" id="round_off"></i> <span class="tw-text-base md:tw-text-lg tw-font-semibold" id="round_off_text">0</span>
