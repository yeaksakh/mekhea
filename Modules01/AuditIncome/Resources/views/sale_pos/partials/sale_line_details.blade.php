<table class="table @if(!empty($for_ledger)) table-slim mb-0 bg-light-gray @else bg-gray @endif" @if(!empty($for_pdf)) style="width: 100%;" @endif>
    ...
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            <td>
                @if($sell_line->product->type == 'combo')
                    <span style="font-weight: bold;">{{ $sell_line->product->name }} (Combo)</span>
                @else
            ...
            @if(!empty($pos_settings['inline_service_staff']))
                <td>{{ $sell_line->service_staff->user_full_name ?? '' }}</td>
            @endif
        ...
        @if($sell_line->product->type == 'combo' && isset($combo_products[$sell_line->id]))
            <tr class="combo-details combo-{{ $sell_line->id }}">
                <td colspan="10" style="padding: 0;">
                    <table class="table" style="margin-bottom: 0; background: #f9f9f9;">
                        <tr>
                            <th width="5%"></th>
                        ...
                        @foreach($combo_products[$sell_line->id] as $combo_product)
                            <tr>
                                <td></td>
                                <td>
                                    {{$combo_product['variation']['product']->name}} 
                                    @if($combo_product['variation']['product']->type == 'variable')
                                        - {{$combo_product['variation']->name}}
                                    @endif
                                    ({{$combo_product['variation']->sub_sku}})
                                </td>
                                <td>
                                    <span class="display_currency" data-currency_symbol="false" data-is_quantity="true">
                                        {{$combo_product['quantity'] * $sell_line->quantity}}
                                    </span> 
                                    {{$combo_product['unit_name']}}
                                </td>
                                <td>
                                    <span class="display_currency" data-currency_symbol="true">
                                        {{$combo_product['variation']->dpp_inc_tax}}
                                    </span>
                                </td>
                                <td>
                                    <span class="display_currency" data-currency_symbol="true">
                                        {{$combo_product['variation']->sell_price_inc_tax}}
                                    </span>
                                </td>
                            </tr>
                        @endforeach
