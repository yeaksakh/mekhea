...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/AuditIncome/Resources/lang/kh/lang.php');
    }

    public function showAuditIncomePermissionForm()
    {
        ...
        $permissions = Permission::where('id', 677)->first();
        ...
        if (!file_exists($this->langFile)) {
            return view('auditincome::AuditIncome.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('auditincome::AuditIncome.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $AuditIncome = AuditIncomeSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('auditincome::AuditIncome.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = AuditIncomeSocial::firstOrNew([
