<div class="container">
    ...
    <div class="header-section">
        <div class="header-content">
            <h2 class="header-title">@lang('documentkeeper::lang.details')</h2>
    ...
    <div class="profile-card" id="print-content">
        ...
        <div class="profile-section" id="categorycontent">
            @if($documentkeeper->category)
            @php
            $imageExtensions = ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'webp'];
            $fileExtension = strtolower(pathinfo($documentkeeper->category->image, PATHINFO_EXTENSION));
            @endphp

            @if(in_array($fileExtension, $imageExtensions))
            <div class="profile-image">
                <img src="{{ asset('uploads/DocumentKeeperCategory/' . basename($documentkeeper->category->image)) }}" alt="Document Image">
            </div>
            @elseif($fileExtension === 'pdf')
            <div class="profile-image">
                <a href="{{ asset('uploads/DocumentKeeperCategory/' . basename($documentkeeper->category->image)) }}" target="_blank">
                    <i class="fas fa-file-pdf"></i>
                </a>
            </div>
            @endif
            <div class="profile-info">
                <h3>{{ $documentkeeper->category->name }}</h3>
                <p>{!! $documentkeeper->category->description !!}</p>
            </div>
            @endif
        </div>
        <div class="profile-section" id="title_1content">
    <h4>@lang('documentkeeper::lang.title_1')</h4>
    <p>{{ $documentkeeper->title_1 }}</p>
</div>
<div class="profile-section" id="file_2content">
    <h4>@lang('documentkeeper::lang.file_2')</h4>
    <div class="profile-image">
        <img src="{{ asset('uploads/DocumentKeeper/' . $documentkeeper->file_2) }}" alt="Document Image">
    </div>
</div>
        <!-- Additional Sections -->
        ...
        <div class="profile-section" id="createdatcontent">
            ...
            <p>{{ $documentkeeper->created_at }}</p>
        </div>

        <!-- QR Code Section -->
        <div class="profile-section" id="qrcontent">
            <h4>@lang('documentkeeper::lang.qrcode')</h4>
