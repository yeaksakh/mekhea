...
class SettingController extends Controller
{
    ...
    public function __construct(ModuleUtil $moduleUtil)
    {
        ...
        $this->langFile = base_path('Modules/DocumentKeeper/Resources/lang/kh/lang.php');
    }

    public function showDocumentKeeperPermissionForm()
    {
        ...
        if (!file_exists($this->langFile)) {
            return view('documentkeeper::DocumentKeeper.setting', ['error' => "File not found: {$this->langFile}"]);
        }

        $lang = include($this->langFile);
        if (!is_array($lang)) {
            return view('documentkeeper::DocumentKeeper.setting', ['error' => "Invalid lang file: {$this->langFile}"]);
        }
        $DocumentKeeper = DocumentKeeperSocial::where('business_id',$business_id)->first();

        $user = auth()->user();
        $languages = ['en' => 'English', 'kh' => 'Khmer']; // Add more languages as needed

        return view('documentkeeper::DocumentKeeper.setting', [
            'roles' => $roles,
...
";
    ...
    public function updateSocial(Request $request)
    {
        ...
        $module = DocumentKeeperSocial::firstOrNew([
