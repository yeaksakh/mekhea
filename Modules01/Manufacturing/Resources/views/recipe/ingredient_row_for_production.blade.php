	...
	<td>
		...
		@php
			...
			if($ingredient['enable_stock'] == 1) {
				// Check both contexts: edit view and AJAX request
				$location_id = isset($production_purchase) 
					? $production_purchase->location_id 
					: request()->get('location_id');
				
				foreach($variation->variation_location_details as $location_detail) {
					if($location_detail->location_id == $location_id) {
						$qty_available = $location_detail->qty_available;
						break;
					}
				}
				
				$max_qty_rule = $qty_available;
