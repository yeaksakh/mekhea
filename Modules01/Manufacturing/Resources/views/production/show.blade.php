...
<div class="modal-dialog" role="document">
    <div class="modal-content" id="productionModalContent">
        <div class="modal-header">
            <button type="button" class="close no-print" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title">@lang( 'manufacturing::lang.production_details' ) (<b>@lang('purchase.ref_no'):</b> #{{ $production_purchase->ref_no }})</h4>
        </div>
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th>@lang('manufacturing::lang.waste_percent')</th>
                                <th>@lang('manufacturing::lang.final_quantity')</th>
                                <th class="price-column">@lang('manufacturing::lang.total_price')</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="4" class="text-right"><strong class="price-label">@lang('manufacturing::lang.ingredients_cost')</strong></td>
                                <td class="price-value"><span class="display_currency" data-currency_symbol="true">{{$total_ingredient_price}}</span></td>
                            </tr>
                            <tr>
                                <td colspan="4" class="text-right"><strong class="price-label">{{__('manufacturing::lang.production_cost')}}:</strong></td>
                                <td class="price-value"><span class="display_currency" data-currency_symbol="true">{{$total_production_cost}}</span> </td>
                            </tr>
                            <tr><td colspan="4" class="text-right"><strong class="price-label">{{__('manufacturing::lang.total_cost')}}:</strong></td>
                                <td class="price-value"><span class="display_currency" data-currency_symbol="true">{{$production_purchase->final_total}}</span></td></tr>
                        </tfoot>
                    </table>
                </div>
            </div>
        ...
        <div class="modal-footer">
            ...
            <button type="button" class="tw-dw-btn tw-dw-btn-primary tw-text-white no-print" onclick="printProductionWithPrices()">
                <i class="fa fa-print"></i> @lang('messages.print')
            </button>

            <!-- Print Without Pricing Button -->
            <button type="button" class="tw-dw-btn tw-dw-btn-warning tw-text-white no-print" onclick="printProductionWithoutPrices()">
                <i class="fa fa-print"></i> print no price
            </button>
...
<!-- Styles for Print -->
<style>
    /* Hide elements temporarily when printing without prices */
    .hide-on-print-temp {
        display: none !important;
    }

    @media print {
        /* Hide price-related elements when printing without prices */
        .print-without-prices .price-column,
        .print-without-prices .price-value,
        .print-without-prices .price-label,
        .print-without-prices .price-column *,
        .print-without-prices .price-value *,
        .print-without-prices .price-label * {
            display: none !important;
...
<script>
    // Print with all details including prices
    function printProductionWithPrices() {
        // Ensure no print-without-prices class is set
        $('#productionModalContent').removeClass('print-without-prices');
        
        $('#productionModalContent').printThis({
            importCSS: true,
            removeInline: false,
            printDelay: 500
        });
    ...
    // Print without any price or cost
    function printProductionWithoutPrices() {
        const $content = $('#productionModalContent');
        
        // Add class to hide prices during print
        $content.addClass('print-without-prices');
        
        // Clone the content for printing
        const $printContent = $content.clone();
        
        // Remove price elements from the cloned content
        $printContent.find('.price-column, .price-value, .price-label').remove();
        
        // Adjust colspan for tfoot rows after removing price column
        $printContent.find('tfoot tr td[colspan="5"]').attr('colspan', '4');
        $printContent.find('.bg-gray[colspan="5"]').attr('colspan', '4');
        
        // Create a temporary container and print
        const $tempContainer = $('<div>').append($printContent);
        ...
        $tempContainer.printThis({
            ...
            afterPrint: function () {
                ...
                $content.removeClass('print-without-prices');
