...
class ProductionController extends Controller
{
    ...
    public function show($id)
    {
        ...
        foreach ($production_sell->sell_lines as $sell_line) {
            ...
            // dump($sell_line->mfg_ingredient_group_id);
    ...
    public function destroy($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                // Get production purchase transaction
                $production_purchase = Transaction::where('id', $id)
                    ->where('business_id', $business_id)
                    ->where('type', 'production_purchase')
                    ->with('purchase_lines')
                    ->firstOrFail();

                // Get related production sell transaction
                $production_sell = Transaction::where('business_id', $business_id)
                    ->where('type', 'production_sell')
                    ->where('mfg_parent_production_purchase_id', $production_purchase->id)
                    ->with('sell_lines')
                ...
                // If finalized, reverse stock movements
                if ($production_purchase->mfg_is_final == 1 && $production_sell) {

                    // Reverse ingredient stock (increase back what was consumed)
                    foreach ($production_sell->sell_lines as $sell_line) {
                        if ($sell_line->enable_stock) {
                            $line_qty = $sell_line->quantity * $sell_line->base_unit_multiplier;

                            // Use updateProductQuantity to increase stock
                            $this->productUtil->updateProductQuantity(
                                $production_sell->location_id,
                                $sell_line->product_id,
                                $sell_line->variation_id,
                                $line_qty, // new quantity (adding back)
                                0,         // old quantity
                                null,
                                false      // already calculated
                            );
                        }
                    }

                    // Reverse finished product stock (decrease what was added)
                    foreach ($production_purchase->purchase_lines as $purchase_line) {
                        $line_qty = $purchase_line->quantity;

                        $this->productUtil->decreaseProductQuantity(
                            $purchase_line->product_id,
                            $purchase_line->variation_id,
                            $production_purchase->location_id,
                            $line_qty,
                            0
                        );
                    }
                ...
                // Delete production sell lines and transaction
                if ($production_sell) {
                ...
                $production_purchase->purchase_lines()->delete();


                // Delete production purchase transaction
                $production_purchase->delete();
